...
export default class ReferCheckbox extends Component {
  static defaultProps = {
    data: {
      title: "表单"
    }
  ...
  methods = {
    ...
    onChange: item => {
      ...
      let hasIndex = active.findIndex((activeItem, activeIndex) => {
        return activeItem.value == item.value;
      });
      if (hasIndex > -1) {
        active.splice(hasIndex, 1);
      } else {
        active.push(item);
      }
  ...
  computed = {
    ...
    renderItem: ({ item, index }) => {
      let boolean = this.state.active.find((activeItem, activeIndex) => {
        return activeItem.value == item.value;
      });
      return (
    ...
    getValue: () => {
      ...
      if (submitActive && submitActive.length > 0) {
        labels = submitActive.map((item, index) => {
          return item.label;
        });
        labels = labels.join(",");
        return labels;
        ...
        <List.Item
          ...
          extra={
            <View
              ...
              <Text style={{ color: "#666666" }} numberOfLines={1} ellipsizeMode="tail">
